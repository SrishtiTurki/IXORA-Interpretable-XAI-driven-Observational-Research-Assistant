...
async def detect_intent(query: str, domain: str = "biomed") -> str:
    ...
    if domain in ["cs", "computerscience", "ai"]:
        ...
        cs_research_keywords = [
            "paper", "publication", "conference", "journal", "arxiv",
            "method", "approach", "algorithm", "model", "architecture",
            "framework", "implementation", "evaluation", "benchmark",
            "experiment", "analysis", "results", "findings", "contribution",
            "related work", "state of the art", "sota"
        ]
        
        # CS-specific analysis keywords
        cs_analyze_keywords = [
            "analyze", "evaluate", "compare", "performance", "accuracy",
            "precision", "recall", "f1", "metrics", "benchmark",
            "optimize", "improve", "speed up", "efficiency", "scalability",
            "complexity", "big o", "time complexity", "space complexity",
            "visualize", "plot", "graph", "chart", "statistics"
        ]
        
        # CS-specific code-related keywords
        cs_code_keywords = [
            "code", "implementation", "function", "class", "module",
            "library", "package", "api", "sdk", "framework",
            "git", "github", "version control", "pull request", "merge",
            "debug", "test", "unit test", "integration test", "ci/cd"
        ]
        
        if any(keyword in query_lower for keyword in cs_research_keywords):
            return "research"
            
        if any(keyword in query_lower for keyword in cs_analyze_keywords):
            return "analyze"
            
        if any(keyword in query_lower for keyword in cs_code_keywords):
            return "analyze"
            
    else:  # Biomedical domain
        ...
        # Biomedical-specific terms
        biomedical_terms = [
            "ph", "temperature", "enzyme", "protein", "cell", "growth", 
...
def _extract_with_regex(sentence: str, domain: str = "biomed") -> Dict[str, Any]:
    """Regex-based parameter extraction as fallback with domain support"""
    params = {}
    ...
    if domain in ["cs", "computerscience", "ai"]:
        ...
        patterns = {
            'learning_rate': [
                (r'(?:learning[\s-]?rate|lr|α|alpha)\s*[=:]?\s*([\d\.e-]+)', 'single', ''),
                (r'lr\s*[=:]?\s*([\d\.e-]+)', 'single', '')
            ],
            'batch_size': [
                (r'(?:batch[\s-]?size|bs)\s*[=:]?\s*(\d+)', 'single', 'samples'),
                (r'batch\s*[=:]?\s*(\d+)', 'single', 'samples')
            ],
            'epochs': [
                (r'(?:epochs?|iterations?|steps?)\s*[=:]?\s*(\d+)', 'single', ''),
                (r'train\s+(?:for\s+)?(\d+)\s+(?:epochs?|iterations?|steps?)', 'single', '')
            ],
            'model_architecture': [
                (r'(?:model|architecture|network|backbone)\s*[=:]?\s*([a-z0-9\-_]+)', 'single', ''),
                (r'(resnet|vgg|inception|bert|gpt|transformer|lstm|rnn|cnn|densenet|efficientnet)', 'single', '')
            ],
            'optimizer': [
                (r'optimizer\s*[=:]?\s*(adam|sgd|rmsprop|adagrad|adadelta|adamw)', 'single', ''),
                (r'using\s+(adam|sgd|rmsprop|adagrad|adadelta|adamw)', 'single', '')
            ],
            'dropout': [
                (r'dropout\s*[=:]?\s*([\d\.]+)', 'single', '')
            ]
        }
    else:  # Biomedical patterns
        patterns = {
            'ph': [
...
async def extract_parameters(query: str, domain: str = "biomed"):
    """
    Main parameter extraction function with domain awareness
    
    Args:
        query: The input text to extract parameters from
        domain: The domain context ('biomed' or 'cs'/'computerscience')
        
    Returns:
        Dict containing extracted parameters and metadata
    """
    # Normalize domain
    domain = domain.lower()
    if domain in ["cs", "ai"]:
    ...
    logger.info(f"Extracting parameters for domain: {domain}")
    
    try:
        # Try NLTK first if available
        if _nltk_ready:
            logger.debug("Using NLTK for parameter extraction")
            params = await extract_parameters_nltk(query, domain)
        else:
            logger.debug("Falling back to regex extraction")
            params = await extract_parameters_regex(query, domain)
        
        # Post-process parameters based on domain
        if domain == "computerscience":
            # Ensure common CS parameters are in the right format
            if 'learning_rate' in params and isinstance(params['learning_rate'], (int, float)):
                params['learning_rate'] = {
                    'value': float(params['learning_rate']),
                    'unit': '',
                    'confidence': 0.9,
                    'source': 'extracted',
                    'context': 'Extracted from query'
                }
                
            # Add domain context to all parameters
            for param in params.values():
                if isinstance(param, dict):
                    param['domain'] = 'computerscience'
...
def select_explainability_method(user_input: str, parameters: Dict[str, Any], domain: str = "biomed") -> str:
    """
    Select the most appropriate explainability method based on the query, parameters, and domain.
    ...
    Returns:
        str: The selected explainability method
    """
    # Normalize domain
    domain = domain.lower()
    if domain in ["cs", "ai"]:
    ...
    if domain == "computerscience":
        # Default method for CS/ML
        method = "attention"
        
        # Check for specific model architectures
        model_arch = parameters.get('model_architecture', {}).get('value', '').lower()
        if any(arch in model_arch for arch in ["transformer", "bert", "gpt"]):
            method = "attention"
        elif any(arch in model_arch for arch in ["resnet", "cnn", "efficientnet"]):
            method = "gradcam"
        
        # Check for specific query patterns
        if any(term in input_lower for term in ["feature importance", "contribution"]):
            method = "shap"
        elif any(term in input_lower for term in ["why", "explain this prediction", "local"]):
            method = "lime"
        elif any(term in input_lower for term in ["attention", "focus", "look"]):
            method = "attention"
            
    else:  # Biomedical domain
        # Default method for biomedical
        method = "shap"
        
        # Check for specific query patterns
        if any(term in input_lower for term in ["why this", "explain this", "how come"]):
            method = "lime"
        elif any(term in input_lower for term in ["statistical", "p-value", "significance"]):
            method = "statistical"
        
        # Parameter-based selection
        if parameters:
            # Check for statistical parameters
            if any(p in parameters for p in ["p_value", "confidence_interval", "odds_ratio"]):
                method = "statistical"
            # Check for time series/sequence data
            elif any(p in parameters for p in ["time_series", "sequence", "temporal"]):
                method = "lime"  # LIME is generally better for sequential data
    
    logger.info(f"Selected explainability method for {domain} domain: {method}")
    return method
