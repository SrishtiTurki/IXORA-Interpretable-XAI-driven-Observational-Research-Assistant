...
import json
from core.analytics import run_shap_analysis, run_lime_analysis, run_bayesian_optimization, run_causal_inference, find_optimal_conditions
...
def task_cpu_optimal(self, user_input: str, params_json: str, domain: str) -> str:
    ...
    try:
        # Extract CS parameters if domain is computerscience
        if domain in ["computerscience", "cs"] and (not params_json or params_json == '{}'):
            cs_params = extract_cs_parameters(user_input)
            if cs_params and 'parameters' in cs_params:
                params = {k: v['value'] for k, v in cs_params['parameters'].items()}
                params_json = json.dumps(params)
            
        params = json.loads(params_json) if isinstance(params_json, str) else params_json
        if not params:
            return json.dumps({"method": "skipped", "reason": "No parameters provided"})
        ...
        elif domain == "computerscience" or domain == "cs":
            optimals.update({
...
def celery_health_check():
    ...
    try:
        # Test CS parameter extraction
        test_cs_extraction = True
        try:
            import asyncio
            from core.cs_parameter_extractor import cs_extractor
            test_params = asyncio.run(cs_extractor.extract_parameters("learning rate 1e-4 batch 32"))
            test_cs_extraction = bool(test_params and 'parameters' in test_params)
        except Exception as e:
            logger.warning(f"CS extractor health check failed: {str(e)}")
            test_cs_extraction = False
            
        return {
            "status": "healthy",
            "broker": CELERY_BROKER_URL is not None,
            "backend": CELERY_RESULT_BACKEND is not None,
            "worker_concurrency": CELERY_WORKER_CONCURRENCY,
            "time_limit": CELERY_TASK_TIME_LIMIT,
            "features": {
                "cs_parameter_extraction": test_cs_extraction,
                "domains_supported": ["biomed", "computerscience"]
            }
        }
    except Exception as e:
