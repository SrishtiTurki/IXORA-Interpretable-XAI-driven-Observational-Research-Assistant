...
from core.model_loader import classify_domain
...
class ChatRequest(BaseModel):
    ...
    domain: Optional[str] = "auto"  # auto, medical, or computerscience
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        if req.domain == "auto":
            # Auto-detect domain
            domain_scores = await classify_domain(req.message)
            domain = max(domain_scores.items(), key=lambda x: x[1])[0]
            logger.info(f"Auto-detected domain: {domain} (scores: {domain_scores})")
        else:
            domain = req.domain.lower()
            
        # Validate domain
        valid_domains = ["biomed", "computerscience"]
        if domain not in valid_domains:
            domain = "biomed"  # Default to biomed for unknown domains
            
        logger.info(f"Using domain: {domain} for query: {req.message[:100]}...")
        
        # Start timing
        import time
        start_time = time.time()
        
        # Call with history and determined domain
    ...
    try:
        ...
        if not parameters:
            ...
            if last_query:
                ...
                try:
                    ...
                    parameters = await extract_parameters(last_query, "biomed")
        ...
        try:
            ...
            causal_result = await asyncio.wait_for(
                run_causal_analysis(parameters, "biomed"),
                timeout=60.0
            )
...
async def causal_analysis(req: CausalRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        causal_result = await run_causal_analysis(req.query, req.parameters or {}, "biomed")
