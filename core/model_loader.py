...
class ModelConfig:
    ...
    DOMAIN_PRIORITIES = {
        ...
        "cs": ["nltk", "embedding", "cs_extractor", "mistral_api", "reward"],
...
class EmbeddingService:
    ...
    def __init__(self, model_loader):
        ...
        self.domain_configs = {
            ...
            "cs": {
                "special_tokens": ["algorithm", "time complexity", "space", "GPU", "batch", "learning rate", "optimizer", "dataset"],
                "dimension": 384
            },
            "general": {
                "special_tokens": [],
...
class UnifiedModelLoader:
    ...
    def _init_model_registry(self):
        ...
        self.models = {
            "nltk": ModelStatus("nltk"),
            "embedding": ModelStatus("embedding"),
            "biomed_extractor": ModelStatus("biomed_extractor"),  # NEW
            "cs_extractor": ModelStatus("cs_extractor"),  # NEW
            "biomistral": ModelStatus("biomistral"),
            "mistral_api": ModelStatus("mistral_api"),
            "reward": ModelStatus("reward")
        }
    ...
    async def _load_critical_models(self, critical_models: list) -> bool:
        """Load critical models first"""
        results = []
        
        for model_name in critical_models:
            if model_name not in self.models:
                logger.warning(f"Unknown critical model: {model_name}")
                continue
            
            try:
                logger.info(f"🔧 Loading critical model: {model_name}")
                instance = await self._load_model(model_name, "general", critical=True)
                results.append(instance is not None)
            except Exception as e:
    ...
    # CS extractor loader
    async def _load_cs_extractor(self):
        """Load computer science parameter extractor"""
        try:
            from core.cs_parameter_extractor import initialize_cs_extractor
            success = await initialize_cs_extractor()
            return success
        except Exception as e:
            logger.error(f"CS extractor load failed: {e}")
            return None
    
    async def _load_model(self, model_name: str, domain: str = "general", critical: bool = False) -> Any:
        """Load a specific model"""
        status = self.models[model_name]
        
        with self._lock:
            if status.loading:
                timeout = ModelConfig.LOAD_TIMEOUTS.get(model_name, 30.0)
                await self._wait_for_load(model_name, timeout)
                return status.instance if status.loaded else None
        ...
        try:
            ...
            loader_func = getattr(self, f"_load_{model_name}")
            ...
            if instance:
                ...
                status.error = f"Loader returned None for {model_name}"
    ...
    async def _warmup_models(self):
        ...
        # Warm up BioMistral
        if self.models["biomistral"].loaded and self.models["biomistral"].instance:
            async def warmup_biomistral():
                try:
                    llm = self.models["biomistral"].instance
                    # Run in thread
                    def generate():
                        return llm("Warmup query", max_new_tokens=2)
                    
                    await asyncio.wait_for(
                        asyncio.to_thread(generate),
                        timeout=5.0
                    )
                    logger.info("✅ BioMistral warmed up")
                except Exception as e:
                    logger.warning(f"BioMistral warmup warning: {e}")
...
async def get_biomistral():
    ...
    return await model_loader.get_model("biomistral")
